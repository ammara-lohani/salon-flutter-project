...
class BookingScreen extends StatefulWidget {
  ...
  const BookingScreen({Key? key, required this.deal}) : super(key: key);
  @override
  ...
  Future<void> _checkSlotAvailability() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$supabaseUrl/rest/v1/booking?select=time&date=eq.$formattedDate'),
        headers: {
          'apikey': supabaseKey,
  ...
  Future<void> _submitBooking() async {
    if (!_formKey.currentState!.validate() || selectedDate == null || selectedTimeSlot == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please fill all fields and select date and time'),
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$supabaseUrl/rest/v1/booking'),
        headers: {
          'apikey': supabaseKey,
          'Authorization': 'Bearer $supabaseKey',
          'Content-Type': 'application/json',
      ...
      if (response.statusCode == 201) {
        ...
        // Show detailed error
        String errorMessage = 'Failed to book appointment (${response.statusCode})';
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text('Deal: ${widget.deal.name}', style: const TextStyle(fontWeight: FontWeight.bold)),
              Text('Date: ${selectedDate!.day}/${selectedDate!.month}/${selectedDate!.year}'),
              Text('Time: $selectedTimeSlot'),
              Text('Name: ${_nameController.text}'),
              const SizedBox(height: 16),
              Text('Thank you booking, See you soon!', style: TextStyle(color: const Color.fromARGB(255, 192, 31, 160))),
            ],
          ),
          actions: [
            TextButton(
            ...
            Container(
              ...
              child: ClipRRect(
                ...
                child: Container(
                  ...
                  child: Stack(
                    children: [
                      if (widget.deal.image != null && widget.deal.image!.isNotEmpty)
                        Positioned.fill(
                          child: Image.network(
                            widget.deal.image!,
                            fit: BoxFit.cover,
